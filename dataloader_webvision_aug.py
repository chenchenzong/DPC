...
class webvision_dataset(Dataset): 
    def __init__(self, root_dir, transform, mode, num_class, pred=[], probability=[], log='',transform_aug=None): 
        self.root = root_dir
        self.transform = transform
        self.mode = mode  
    ...
    def __getitem__(self, index):
        if self.mode=='labeled':
            ...
            img3 = self.transform_aug(image) 
            img4 = self.transform_aug(image)
...
class webvision_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, transform_aug=self.transform_train_aug, mode="labeled",num_class=self.num_class,pred=pred,probability=prob,log=self.log)              
            ...
            unlabeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, transform_aug=self.transform_train_aug, mode="unlabeled",num_class=self.num_class,pred=pred,log=self.log)                    
